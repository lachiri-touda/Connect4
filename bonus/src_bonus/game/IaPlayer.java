...
public class IaPlayer extends Player {
	...
	private String name ;
	private int score;
	// ai power to calculate 
	private final static int AIPOWER = 6;
	...
	public boolean insertInColonne(Grid g , int column , int type) {
		boolean valide = true;
	...
	public boolean play( Grid g ,Game game) {
		...
		else if (this.name.equals("minimax")) {
		...
		if ( (0 > move ) || (move >= g.getWidth() )) {
	        ...
	        valide=insertInColonne(g,move,numPlayer);
		 ...
		 int move=0;
		    ...
		    for ( int j = 0; j < g.getWidth(); ++j) {
		      ...
		      if (!isFull(g,j)) {
		    	  Grid copy=new Grid();
		          copy.copyGrid(g);
		    	  insertInColonne(copy,j,numPlayer);
		        int score = minimax(copy, AIPOWER, false);
